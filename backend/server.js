...
const { generateAutofillWorkouts, clearWorkoutsForGoal } = require('./weeklyGoal_handlers');
...
runplannerRoutes.route("/clearworkoutsforgoal").post(function (req, res) {
    proceedIfUserExists(req.body.userID,
        (user) => {
            clearWorkoutsForGoal(user._id, req.body.goalID, (deletedIDs) => {
                if (deletedIDs) {
                    res.status(200).json({
                        deleted: deletedIDs,
                        message: deletedIDs.length + ' autogenerated workouts deleted',
                    });
                }
            });
        },
        () => { res.status(400).send('Clearing autogenerated workouts failed') })
});
