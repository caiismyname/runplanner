...
const generateAutofillWorkouts = (existingWorkouts, goalPayload, userConfig, ownerID, callback) => {
    ...
    if (userConfig.autofillConfig.distribution === autofillDistributions.EVEN) {
        // const dailyMilage = roundToOneDecimal((goalPayload.goalValue - allocatedTotal) / numDaysToFill);
        // const mileages = Array(numDaysToFill).fill(dailyMilage);

        // Alt method, where milage comes out to half or whole values
        ...
        const numDaysWithAdjustment = Math.ceil(diff / 0.5) - (diff > 0 ? 0 : 1);
        ...
        for (let date in days) {
            ...
            if (days[date].length === 0) {
                ...
                workout.payload.milage.goal = mileages[mileageIdx];
