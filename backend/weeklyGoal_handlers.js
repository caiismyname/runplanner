...
const generateAutofillWorkouts = (existingWorkouts, goalPayload, userConfig, ownerID, callback) => {
    ...
    } else if (userConfig.autofillConfig.distribution === autofillDistributions.RANDOM) {
...
const clearWorkoutsForGoal = (userID, goalID, callback) => {
    WeeklyGoals.findById(goalID, function (err, goal) {
        getWorkoutsForOwnerForDateRange(
            userID,
            goal.payload.startDate,
            goal.payload.endDate,
            (workouts) => {
                const idsToDelete = workouts.map(workout => workout.id);
                deleteWorkouts(idsToDelete, userID, (deleted) => {
                    callback(deleted);
                });
            },
            {'payload.creationType': creationTypes.AUTOFILLWEEK}
        );
    })
}
...
exports.clearWorkoutsForGoal= clearWorkoutsForGoal;